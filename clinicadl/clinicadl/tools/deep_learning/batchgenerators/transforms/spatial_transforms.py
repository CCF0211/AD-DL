...
class Rot90Transform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        if len(data.shape) == 4:
            data = np.array(data.unsqueeze(0))
        ...
        data_dict[self.data_key] = torch.from_numpy(data).squeeze(0)
        if seg is not None:
            data_dict[self.label_key] = torch.from_numpy(seg).squeeze(0)
        return data_dict
...
class ChannelTranslation(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        data_dict[self.data_key] = torch.from_numpy(ret_val[0]).squeeze(0)
        return data_dict
...
class SpatialTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        data_dict[self.data_key] = torch.from_numpy(ret_val[0]).squeeze(0)
        if seg is not None:
            data_dict[self.label_key] = torch.from_numpy(ret_val[1]).squeeze(0)
